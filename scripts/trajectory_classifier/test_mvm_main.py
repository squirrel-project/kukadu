...
## !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  pselect=1.0
  params.solver.niter=20
  xeps=0.00001
  ## rmatrix=mvm_random_matrix.random_matrix()

  xrank=np.array([10,20,50,100,200])
  lrank=len(xrank)

  nparacc=4   ## nrank,random_estimation, rmse, time
  ...
  for irank in range(lrank):
    ...
    if irandtype==0:
      x0=np.random.randn(nrow,nrank)
      z0=np.random.randn(nrow,nrank)
      srandtype='Gaussian'
    elif irandtype==1:
      pshape=3.0
      xi=np.random.rand(nrow,nrank)
      x0=xi**(1/pshape)
      x0=1/(x0+xeps)
      x0=x0-np.mean(x0)
      xi=np.random.rand(nrow,nrank)
      ...
      z0=1/(z0+xeps)
      z0=z0-np.mean(z0)
      srandtype='Power Law'+'('+str(pshape)+')'
    elif irandtype==2:
      pshape=1.0
      xi=np.random.rand(nrow,nrank)
      x0=-np.log(1-xi+xeps)/pshape
      x0=x0-np.mean(x0)
      xi=np.random.rand(nrow,nrank)
      z0=-np.log(1-xi+xeps)/pshape
      z0=z0-np.mean(z0)
      srandtype='Exponential'
    elif irandtype==3:
      x0=np.exp(np.random.randn(nrow,nrank))
      x0=x0-np.mean(x0)
      z0=np.exp(np.random.randn(nrow,nrank))
      z0=z0-np.mean(z0)
      srandtype='LogNormal'
    ...
    if nrow<=1000:
      x00=np.dot(x0,x0.T)
      z00=np.dot(z0,z0.T)
      xz0_err2=np.sqrt(np.mean((x00-z00)**2))
      xz0_err1=np.mean(np.abs(x00-z00))
      xz0_err10=np.median(np.abs(x00-x00))
      print('Random estimate RMSE+MAE:',xz0_err2,xz0_err1,xz0_err10)
      xsummary[irank,0]=nrank
      xsummary[irank,1]=xz0_err2

    rmatrix=mvm_random_matrix.cls_random_matrix()
    (xdata,nrow2,ncol2)=rmatrix.mvm_form_prod_reorder(x0,xfactors)

    ## xdata=rmatrix.mvm_form_prod(x0,pselect=pselect)
    ## nrow2=nrow
    ## ncol2=ncol
...
#           xconfusion3[irepeat,ifold]=cEval.xconfusion3
    ...
    xsummary[irank,2]=np.mean(xresultte[:,:,0])
    xsummary[irank,3]=xsolvertime/(nrepeat0*nfold0)                          
