...
int main(int argc, char** args) {
    ...
    for(double height = 3; height < 12; height += 2) {
        ...
        for(double slope = 1; slope < 4; ++slope) {
            ...
            SegmentationTestingRewardComputer comp(height, slope, 1, 100.0);
...
void testPower(ros::NodeHandle* node) {
    ...
    shared_ptr<GaussianObstacleRewardComputer> reward(new GaussianObstacleRewardComputer(2, 2.0, 3, 100.0));
